...
class DCGAN():
    def __init__(self, load=False):
        ...
        if load == True:
            self.discriminator.load_weights('saved_model/dcgan_discriminator_weights.hdf5')
        ...
        if load == True:
            self.generator.load_weights('saved_model/dcgan_generator_weights.hdf5')
    ...
    def save_imgs(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i, j].axis('off')
                cnt += 1
        fig.savefig("images_rgb/mnist_%d.png" % epoch)
        plt.close()
