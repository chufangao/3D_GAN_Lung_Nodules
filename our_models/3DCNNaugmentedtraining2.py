...
# print total shape of train and val
...
for i in experiment_trials:
    ...
    if i[0] != 0:
        the_noise = np.random.normal(0,1,(int(i[0]*generate_quantity), latent_dim))
        fake_pos_data = example_generator.predict(the_noise)
        fake_pos_data = denormalize_img(fake_pos_data)
        # make labels for fake pos data according to experiment
        fake_pos_label = []
